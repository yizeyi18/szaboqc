...
这个方程的重点在于，
它包含着单激发
、双激发和三激发的系数，因而也将这三者耦合起来。若继续以上的流程，依次在\autoref{4.7}上左乘$\bra{\Psi_{ab}^{rs}},\bra{\Psi_{abc}^{rst}}$等，就能得到一组从低到高的方程，必须同时求解这组方程才可得到关联能。假若要把所有可能的激发都纳入，那这组耦合方程数目会非常多。这就是full CI矩阵本身很大的另外一种说法。解释完我们目前所推出的形式理论之后，我们接下来把它运用到极小基$\hd$上，如此就回到如何把CI矩阵截断到可运算的大小上面。
\exercise{
