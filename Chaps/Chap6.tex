\chapter{多体微扰理论}
\label{ch:6}
组态相互作用(CI)是一种超越\hft 的系统化方法.
它的一大优势是它变分(即在各个等级下都给出能量的上界),
但它也有一大缺陷是它的尺寸一致性只存在于考虑试探波函数的全部激发时(也就是说,
full CI).
另一种获取相关能的系统化方法是微扰论(perturbation theory, PT).
微扰论不变分, 但是在各个等级下都有尺寸一致性.
这种方法中, 总\ha 被分为两块:
一块零阶部分, $\hs _0$, 有已知的特征函数、特征值；
另一块是扰动, $\vs$.
然后精确能量就可以表达为无穷多项贡献之和, 其中项的复杂度递增.
这些贡献的表达包括$\hs _0$的特征值和$\hs _0$特征函数间微扰得到的矩阵元.
把由$n$个矩阵元乘起来所得的项打包起来就是$n$-阶微扰能量.
如果$\hs _0$选得合适, $\vs$就会很小、
微扰展开(一阶、二阶\ldots, $n$-阶能量的和)也会收敛得很快.

本章中, 我们将讨论一种以 Rayleigh 和 \sch 命名的微扰形式(RSPT).
因为我们关注的是相关能的微扰展开,
所以选取\hft~\ha 作为零阶\ha .
用这种$\hs _0$的RSPT最早由C.~\mo 与M.~S.~Plesset引进$N$-电子系统,
因此它有时候也被称作\mo -Plesset微扰论(MPPT).

这种方法也被称为多体微扰(many-body perturbation theory, MBPT),
因为它在研究无限系统的物理学家间流行.
显然, 要研究这种大系统就需要尺寸一致的理论.
K. A. Bruneckner首先猜想MBPT在各阶上都有尺寸一致性,
当时他在研究无限核物质.
高阶MBPT能量的标准表达里有些项正比于粒子数\textit{平方} \footnote{
尺寸一致的方法中能量应当(近似)正比于粒子数,
否则两个无相互作用的系统摆在一块就会凭空多出些东西来}.
Brueckner能证明前几阶里这些坏项会抵消于是各阶多体微扰都尺寸一致.
但是, 他证不出来所有阶都这样.
为了证明这个定理, J.~Goldstone受R. P. Feynman在量子电动力学上的工作启发,
提出了一个绘图的或者说图论的方法来表示多粒子系统能量的RS微扰展开的代数形式.
他发现各阶中这些正比于粒子数平方的坏项的图表示都由不相连的几块组成,
换言之, 这些图\textit{不连接}.
然后他证明了这些不连接项在各阶中都抵消.
这就是著名的\textit{连接簇理论}(linked-cluster theorem),
它指出多体系统能量的微扰展开完全可表示为连通图.
它向我们保证\mo -Plesset微扰展开的尺寸一致性无关于它截断到几阶.

Goldstone的图方法由H.~P.~Kelly首先应用到原子上, 当时他还是个研究生.
他没有计算只截断到某阶(比如, 二阶)、忽略高阶项的微扰能,
而是通过在高至无穷阶上求特定图的和, 近似考虑它们在所有阶微扰中的贡献.
换言之, 他拿到的近似相关能里不仅有高至一定阶(比如, 二阶)的完整贡献,
还有更高阶项的特定部分.
用这种方法, 他首次完成了一项计算,
后来该计算被证明与独立电子对近似(independent electron pair approximation,
IEPA)基本等价.

本章中, 我们将对图解微扰理论作简单介绍. 
不过, 本章的编排方式使其无需学习图解也能阅读.
讨论图的章节标了星号, 跳过它们不影响连贯性.
任何用图的方法可得的结果, 用代数法也都能得出来.
在量子化学中, 是否认为图解有用且能提供有价值的观点(如提示特定的近似方法),
本质上只是个偏好问题.
我们希望这种简化的讲解能鼓励部分读者进一步学习这些技术.

本章由推导一至三阶RSPT能量的标准表达式开始,
它们可用于各种量子系统.
\autoref{sec6.2}介绍一种用图表示各阶中各种代数项的方法.
我们会从只有两个本征态的简单系统起步,
然后把结论推广到任意多个态的系统.
然后我们会讲$N$-电子系统能量的微扰展开,
以\hft 描述作为我们的起点.
\autoref{sec6.3}处理一种简单情况:
微扰中只含单粒子相互作用.
\autoref{sec6.4}讨论对应的图表示.
\autoref{sec6.5}讲含有二粒子相互作用的微扰,
并推导相关能的微扰展开式.
\autoref{sec6.6}研究尺寸一致性问题,
并将RSPT应用于$N$个极小基$\hd$组成的标准模型.
\autoref{sec6.7}讨论多体微扰的图表示,
最后的\autoref{sec6.8}展示一些描述性算例.
...
\begin{subequations}\label{6.4}
	\begin{align}\label{6.4a}
...
否则总可以进行中间归一化. 
将\autoref{6.4b}左乘$\bra{i}$, 
得到
\begin{align}
\braket{i|\Phi_i}=\braket{i|i}+\lambda\braket{i|\Psi_i^{(1)}}+\lambda^2\braket{i|\Psi_i^{(2)}} + \cdots = 1
\end{align}
上式对于任意$\lambda$取值恒成立,
等号两边$\lambda ^n$的系数就一定相等,即
\begin{align}
\braket{i|\Psi_i^{(n)}} = 0~~~ \hfill n=1,2,3,\ldots
\label{6.6}
\end{align}
将\autoref{6.4a},\autoref{6.4b}代入\autoref{6.1}, 
得到:
\begin{align*}
...
限定$\lambda^n$的系数相等,
有
\begin{subequations}\label{6.7}
	\begin{align}
		\mathscr{H}_0|i\rangle=& E_i^{(0)}|i\rangle & & n=0 \\\label{6.7b}
		\mathscr{H}_0\left|\Psi_i^{(1)}\right\rangle+\mathscr{V}|i\rangle=& E_i^{(0)}\left|\Psi_i^{(1)}\right\rangle+E_i^{(1)}|i\rangle & & n=1 \\\label{6.7c}
		\mathscr{H}_0\left|\Psi_i^{(2)}\right\rangle+\mathscr{V}\left|\Psi_i^{(1)}\right\rangle=& E_i^{(0)}\left|\Psi_i^{(2)}\right\rangle+E_i^{(1)}\left|\Psi_i^{(1)}\right\rangle+E_i^{(2)}|i\rangle & & n=2 \\\label{6.7d}
...
等等. 
每个式子左乘$\ket{i}$, 
由正交关系\autoref{6.6}可得如下各阶能量\footnote{
须注意结合律与$\bra{i}\hs_0$ = $(\hs_0 \ket{i})^\dagger$ .
}:
\begin{subequations}\label{6.8}
\begin{align}\label{6.8a}
E_i^{(0)} &= \braket{i|\hs_0|i} \\\label{6.8b}
E_i^{(1)} &= \braket{i|\vs|i} \\\label{6.8c}
E_i^{(2)} &= \braket{i|\vs|\Psi_i^{(1)}} \\\label{6.8d}
E_i^{(3)} &= \braket{i|\vs|\Psi_i^{(2)}}
\end{align}
\end{subequations}
剩下的事情就是从\autoref{6.7}解出$\ket{\Psi_i^{(n)}}$, 
然后用\autoref{6.8}确定$n$阶能量.
...
(E_i^{(0)} - \hs_0 )\ket{\Psi_i^{(1)}} = (\vs - E_i^{(1)})\ket{i} = (\vs - \braket{i|\vs|i})\ket{i}
\end{align}
上式不再是一个本征方程, 
而成为了一个非齐次微分方程(更广义地, 
成为了积分微分方程). 
解该方程的一种办法是将$\ket{\Psi_i^{(1)}}$用$\hs_0$的本征函数展开(设$\hs_0$的本征函数完备):
\begin{align*}
...
\end{align}
为得到二阶能量, 
将\autoref{6.10}代入\autoref{6.8c}可以得到:
\begin{align*}
E_i^{(2)} = \braket{i|\vs|\Psi_i^{(1)}} = \sideset{}{'}\sum_n\braket{i|\vs|n}\braket{n|\Psi_i^{(1)}}
\end{align*}
使用\autoref{6.11}可以求出$\braket{n|\Psi_i^{(1)}}$, 
所以最终有
...
\end{align*}
最后再代入\autoref{6.10}\autoref{6.11}, 
就得到
\begin{align}
...
此处给出这种办法的一个应用. 
考虑一个仅有两个态的简单量子体系, 
这两个态记作$\ket{I},\ket{II}$, 都是哈密顿量$\hs$的本征函数:
\begin{align*}
...
现在想得到精确的基态能量$\es_1$. 
假设零阶态非简并, 
而且$E_1^{(0)}$是两个特征值里较小的那个(即$E_1^{(0)} < E_2^{(0)}$). 
我们先用精确的办法得出解, 
然后再和微扰解作比较. 
为求解方程
\begin{align}
...
可以证明
\begin{align}
\es_1 = E_1^{(1)} + \lambda E_1^{(1)} + \lambda^2 E_1^{(2)} + \lambda^3 E_1^{(3)} + \lambda^4 E_1^{(4)} + \cdots
\end{align}
其中
\begin{subequations}
	\begin{align}
	...
	E_1^{(3)} &= \frac{V_{12} V_{22} V_{21} }{(E_1^{(0)} - E_2^{(0)})^2} 
                   - \frac{V_{12} V_{21} V_{11} }{ (E_1^{(0)} - E_2^{(0)})^2 }\\
	E_1^{(4)} &= \frac{V_{12} V_{21} V_{11}^2 }{(E_1^{(0)} - E_2^{(0)})^3} 
                   - \frac{2 V_{12} V_{22} V_{21} V_{11} }{(E_1^{(0)} - E_2^{(0)})^3}
                   + \frac{V_{12} V_{22}^2 V_{21} }{(E_1^{(0)} - E_2^{(0)})^3}
                   - \frac{ (V_{12} V_{21})^2 }{(E_1^{(0)} - E_2^{(0)})^3}
	\end{align}
\end{subequations}

最终我们能看到,
由RS微扰理论出发所得的一、二、三阶能量的\autoref{6.8b} \autoref{6.12}
\autoref{6.15}与以上式中的$E_1^{(n)}\,(n=1,2,3)$相等. 
为理解这一点, 
只需注意到两态问题中仅有一个中间态$\ket{n} = \ket{2}$. 
由于微扰理论的公式也是由形式Taylor展开所得, 
出现这样的对应也是可期的. 
...
即使是最简单的两态问题,
其高阶能量公式也相当复杂.
本章的主要目标是用Hartree-Fock基态作为零阶态来得到相关能的微扰展开公式. 
在这种情况下, 
高阶能量的表达式会更复杂. 
因此若能找到某种相对简单的方法, 
将高阶展开中的各项归类并表示出来, 
...
然后再``猜出''将图形转化为数学式子的规则, 
并展示其正确性.
接下来会推广之前的图形及规则.
每一步都会检查由图形和代数方法所得结果一致. 
我们希望这种办法能让习惯并认可这些图形.


微扰展开的图形表示首先出现在量子电动力学中. 
Feynman首创了这种办法, 
并在他的Nobel奖获奖演说\endnote{R. P. Feynman, The development of the space-time view of quantum electrodynamics. \textit{Science} \textbf{153}: 699 (1966). 这是一篇风趣又启示人的文章}
上介绍了其想法的演变发展. 
...
有一条线出去.
对应的矩阵元就是
$\braket{\textbf{入}\text{线标签}| \vs |\textbf{出}\text{线标签} }$.


回忆$n$阶能量表达式中的各项: $n=1,2,3,4$的分数线上方都是$n$个$\vs$矩阵元的乘积. 
既然每个矩阵元都可由一个点和出入的线来表示, 
...
\begin{enumerate}
	...
	\end{tikzpicture}\]
	\item \textit{将所有点用一根线串成环, 保证每个点都连有一出一入.}
	\item \textit{列出所有不同的连接方式. 两图相同的条件是, 每对相同的点在两图中的前、后点分别相同.}
\end{enumerate}

第二个规则强制我们的图只能是完全连起来的, 
或者叫\emph{连通}的(linked), 也就是说, 它们连成一个整体. 
如下的不连通图:
\begin{equation*}
...
\draw[dashed] (-2,.5)node[left]{$C$}--(2,.5)
              (-2,1.5)node[left]{$B$}--(2,1.5)
              (-2,2.5)node[left]{$A$}--(2,2.5);
\begin{scope}
\path [draw=blue,postaction={on each segment={mid arrow}}]
(0,0)..controls+(.5,.5)and+(.5,-.5)..node[right]{2}(0,2) % n=4, 5th
(0,2)..controls+(.25,-.25)and+(.25,.25)..node[left, yshift=3]{1}(0,1)
(0,1)..controls+(-.5,.5)and+(-.5,-.5)..node[right]{2}(0,3)
(0,3)..controls+(-.75,-.75)and+(-.75,.75)..node[left, yshift=3]{1}(0,0);
\end{scope}
\end{tikzpicture}
\end{equation*}
上方的两点各为分子贡献一个因子$\braket{2|\vs|1}=V_{21}$, 
剩余的点各贡献$\braket{1|\vs|2}=V_{12}$. 
虚线$A,C$各都为分母贡献一因子$E_1^{(0)} - E_2^{(0)}$, 
虚线$B$贡献一个因子$(2E_1^{(0)} - 2E_2^{(0)})$. 
此外, 
该图有2条空穴线($h=2$), 1个闭合圈图($l=1$), 
所以要将式子乘以$(-1)^3$. 
所以有
\begin{equation*}
...
\autoref{t6.1}中按照之前的规则计算出了所有的二、三、四阶图对应的代数式. 
按习惯, 
一阶泡泡图定为
\begin{equation*}
\begin{tikzpicture}[baseline={(current bounding box.center)},scale=1]
...
\begin{align}
	...
	\end{tikzpicture} & = \sum_{n\neq 1}\frac{V_{1n}V_{n1}}{E_1^{(0)} - E_n^{(0)}} = \sideset{}{'}\sum_n \frac{V_{1n}V_{n1}}{E_1^{(0)} - E_n^{(0)}}\\
	\begin{tikzpicture}[baseline={(current bounding box.center)},scale=.6] 
\path [draw=blue,postaction={on each segment={mid arrow}}] 
(4.5,0) node{\dian}
arc (225:180:2.12cm) node[left,midway]{$m$}  node{\dian}
...
\end{tikzpicture} 
& = \sideset{}{'}\sum_{nm} \frac{V_{1m}V_{mn}V_{n1}}{(E_1^{(0)} - E_m^{(0)})(E_1^{(0)} - E_n^{(0)})}
    - \sideset{}{'}\sum_n \frac{V_{11}V_{1n}V_{n1}}{(E_1^{(0)} - E_n^{(0)})^2}
\end{align}
\end{subequations}
这两个表达式与之前所得到的二阶、三阶能量\autoref{6.12} \autoref{6.15}的基态($i=1$)完全相同. 
如果现在要得到某个态$i$的微扰表达式, 
而且$i$不一定是最低能态, 
我们要如何处理? 图会是什么样? 我们很容易能够验证, 
如果将空穴线标为$i$, 
粒子线标为$m,n,k$, 
其取值为$1,2,\ldots,i-1,i+1,\ldots,N$, 
那么答案就与之前的相同. 
...
\end{equation*}
其中指标$m,n,k$的值不取$i$. 
用\autoref{sec6.1}介绍的方法得出四阶能量的代数表达式并与图形方法所得的结果比较.
...
+\cdots
\end{align}  
利用\autoref{t6.1}和\autoref{ex:6.1}中的五阶图表达式, 
我们可以得到
...
\Delta = &  \frac{V_{12}V_{21}}{E^{(0)}_1 - E_2^{(0)}}\Bigg[ 1 
    + \left(\frac{V_{22}-V_{11}}{E_1^{(0)} - E_2^{(0)}}\right) 
    + \left(\frac{V_{22}-V_{11}}{E_1^{(0)} - E_2^{(0)}}\right)^2 \notag\\
         & + \left(\frac{V_{22}-V_{11}}{E_1^{(0)} - E_2^{(0)}}\right)^3 +\cdots \Bigg]
\end{align}
注意到方括号中的式子是一几何级数. 
使用公式
\begin{align*}
(1-x)^{-1} = 1+x+x^2+x^3+\cdots
\end{align*}
则级数\autoref{6.26}求和后就成为
\begin{align}
\Delta = &  \frac{V_{12}V_{21}}{E^{(0)}_1 - E_2^{(0)}}\left[
        \frac{1}{1-\cfrac{V_{22}-V_{11}}{E_1^{(0)} - E_2^{(0)}}} \right]
   = \frac{V_{12}V_{21}}{(E_1^{(0)} + V_{11}) - (E_2^{(0)} + V_{22})}
\end{align}
对这组图求和到无穷阶得到的表达式类似于二阶微扰的结果, 
但不同指出在于它有\emph{偏移的能量分母(shifted energy denominator)}. 
...
再次注意到上面所得的结果就是具有偏移的能量分母的二阶(微扰能量)表达式. 
我们之后会知道, 
...
设想已经得到一组自旋轨道与轨道能量作为$h_0$的本征函数和本征值,
...
然后将结果对所有占据自旋轨道求和. 
由于\autoref{sec6.1}中的一般性理论可以用于单粒子零阶\ha (即$h_0$)和单粒子微扰, 
我们立马得到：
\begin{align}
\epsilon_a & = \epsilon_a^{(0)} + \braket{a|v|a}
    + \sideset{}{'}\sum_i\frac{\braket{a|v|i}\braket{i|v|a}}{\epsilon_a^{(0)}-\epsilon_i^{(0)}}
    + \cdots\notag\\
	       & = \epsilon_a^{(0)} + v_{aa}
    + \sideset{}{'}\sum_i\frac{v_{ai}v_{ia}}{\epsilon_a^{(0)}-\epsilon_i^{(0)}}
    + \cdots
\end{align}
对$i$的求和可分为两部分, 
一部分是对所有粒子轨道求和, 
...
\end{align}
当下我们要得到精确能量的微扰展开, 
那么将\autoref{6.40}代入\autoref{6.38}可得
\begin{align}
...
\end{align}
\end{subequations}
可以看见$\ket{\Psi_0}$的能量(见\autoref{6.34})等于零阶与一阶能量之和(即$E_0 = E_0^{(0)}+E_0^{(1)}$).


\autoref{6.43}中的求和遍及所有占据(空穴)和非占据(粒子)自旋轨道. 
由于矩阵元$v_{ij} = \braket{i|v|j}$仅当自旋轨道$i,j$自旋相同时非零, 
所以对于闭壳层体系我们可以将该式重新写为对空间轨道的求和再乘以$2$:
\begin{subequations}\label{6.44}
	\begin{align}\label{6.44a}
	E_0^{(0)} & = 2\sum_a^{N/2}\epsilon_a^{(0)}\\\label{6.44b}
	E_0^{(1)} & = 2\sum_a^{N/2}v_{aa} \\\label{6.44c}
...
E_0^{(2)} = \sum_{ar}\frac{v_{ar}v_{ra}}{\epsilon_a^{(0)} - \epsilon_r^{(0)}}
\end{align*}
你可以从二阶能量的一般表达式(\autoref{6.12})开始, 
将其用在$N$-电子体系中：
\begin{align*}
...
\end{align*}
\Next
用\autoref{6.15}中的一般表达式计算三阶能量$E_3^{(0)}.$
\begin{enumerate}[a.]
	\item 证明
		\begin{align*}
		B_0^{(3)} = -E_0^{(1)}\sideset{}{'}\sum_n
                    \frac{|\braket{\Psi_0|\vs|n}|^2}{(E_0^{(0)} - E_n^{(0)})^2}
                    = - \sum_{abr}\frac{v_{aa}v_{rb}v_{br}}{(\epsilon_b^{(0)} - \epsilon_r^{(0)} )^2}
		\end{align*}
	\item 证明
		\begin{align*}
		A_0^{(3)} = \sideset{}{'}\sum_{nm}
                    \frac{\braket{\Psi_0|\vs|n}\braket{n|\vs|m}\braket{m|\vs|\Psi_0}}{(E^{(0)}_0 - E^{(0)}_n)(E^{(0)}_0 - E^{(0)}_m)}
                    = \sum_{abrs}\frac{v_{ar}v_{sb}\braket{\Psi_a^r|\vs|\Psi_b^s}}{(\epsilon^{(0)}_a - \epsilon^{(0)}_r)(\epsilon^{(0)}_b - \epsilon^{(0)}_s)}
		\end{align*}
	\item 证明
		\begin{align*}
		\begin{array}{clc}
	...
	\begin{align*}
	E_0^{(3)} = A_0^{(3)} + B_0^{(3)}
            = \sum_{ars} \frac{v_{ar} v_{rs} v_{sa}}{(\epsilon^{(0)}_a - \epsilon^{(0)}_r)(\epsilon^{(0)}_a - \epsilon^{(0)}_s)}
            - \sum_{abr} \frac{v_{ra} v_{ab} v_{br}}{(\epsilon^{(0)}_a - \epsilon^{(0)}_r)(\epsilon^{(0)}_b - \epsilon^{(0)}_r)}
	\end{align*}
	\item 证明对于闭壳层体系
	\begin{align*}
...
为说明上述方法我们用微扰论重新处理这个问题. 
回忆共振能的定义——精确总能量(对角化H\"uckel矩阵并将所有占据轨道能量加和)与$N/2=n$个局域烯烃单元的能量之差. 
...
\end{align}
那么烯烃轨道就是$h_0$的本征函数：
\begin{subequations}\label{6.46}
	\begin{align}
	h_0\ket{i} &= (\alpha+\beta)\ket{i} = \epsilon_i^{(0)}\ket{i}\\
	h_0\ket{i^*} &= (\alpha-\beta)\ket{i^*} = \epsilon_{i^*}^{(0)}\ket{i^*}
	\end{align}
\end{subequations}
为之后参考我们先指出$\epsilon_i^{(0)} - \epsilon^{(0)}_{j^*}=2\beta$对任意$i,j$恒成立. 
微扰项的非零矩阵元为
\begin{subequations}\label{6.47}
	\begin{align}\label{6.47a}
	\braket{i|v|(i\pm 1)^*} & = \pm\beta/2\\\label{6.47b}
	\braket{i|v|(i\pm 1)} & = \beta/2\\\label{6.47c}
...
有了微扰项$v$的矩阵元\autoref{6.47}和零阶轨道能量\autoref{6.46}我们就能计算微扰能量. 
先从二阶能量开始. 
我们处理的为闭壳层体系, 
相应的式子就是\autoref{6.44c}:
\begin{align}
E_0^{(2)} = 2\sum_{ar}^{N/2}\frac{v_{ar}v_{ra}}{\epsilon^{(0)}_a - \epsilon^{(0)}_r} = 2 \sum_{ar}^{N/2}\frac{\braket{a|v|r}\braket{r|v|a}}{\epsilon^{(0)}_a - \epsilon^{(0)}_r}
\end{align}
指标$a$取遍全部$n$个占据烯烃轨道$\ket{i}$, 
指标$r$取遍总共$n$个未占轨道$\ket{j^*}$, 
因此我们有
\begin{align}\label{6.50}
E_0^{(2)} = 2\sum_{i=1}^{n}\sum_{j=1}^{n}\frac{\braket{i|v|j^*}\braket{j^*|v|i}}{\epsilon^{(0)}_i-\epsilon^{(0)}_{j^*}} = \frac{1}{\beta}\sum_{i=1}^{n}\sum_{j=1}^{N}\braket{i|v|j^*}\braket{j^*|v|i}
\end{align}
式中我们用到了轨道能之间的差为$2\beta$. 
将$i$固定, 
很容易计算对$j$的求和, 
因为只有当$j=i\pm1$时矩阵元才不为零(见\autoref{6.47a}). 
因此
...
对于更大的体系, 
这个结果的质量会更好. 
比较\autoref{6.48}和\autoref{6.51}可见, 
$N$很大的时候, 
二阶能量可达精确结果的91.
5\%.
...
现在来计算三阶能量. 
适用于闭壳层体系的表达式在\autoref{ex:6.4}(e)中给出：
...
\draw (ip1)--node[above]{$-$}(ip2);
\draw (im1)--node[below]{$-$}(im2);
\draw (ip2)--node[above]{$-$}(ie);
\draw (im2)--node[below]{+}(ie);
\draw[dashed] (ie)--(is);
\end{tikzpicture}
\end{align*}
...
\end{align*}
用完全相同的方式我们可以证明\autoref{6.52}中第二项等于$3\beta/8$, 所以总的三阶能量就变成
\begin{align}
E_0^{(3)}(\text{苯}) = \frac{3\beta}{4}
\end{align}
因此苯的共振能算到三阶就是$2.25\beta$(即确切值的113\%). 
\autoref{ex:6.7}中用图技术给出了四阶能量
\begin{align}
...
\exercise{
证明\autoref{6.52}中第二项在苯中等于$\frac{3}{8}\beta$. 
...
\end{align*}
(请参见, 如 L. Salem, \textit{Molecular Orbital Theory of Conjugated Systems},
Benjamin, New York, 1966, pp. 498-500).
...
\begin{enumerate}[a.]
	...
	\end{align*}
	可以先用一般的表达式写出H\"uckel矩阵, 对角化之后将占据轨道能量加和. 注意当$\beta_1=\beta_2=\beta$时我们就回归到以前的结果($6\alpha+8\beta$). 
	...
	(1+y)^{1/2} = 1 + \tfrac{1}{2}y -\tfrac{1}{8}y^2 + \tfrac{1}{16}y^3 - \tfrac{5}{128}y^4 + \cdots\quad|y|<1
 	...
 	下面绘出了到$M$阶的共振能对$M$的函数. 可以看到它振荡收敛到确切值$2\beta$. 以上这种计算$n=2,3,4$时$E_0^{(n)}$的方法当$n$变得很大时就会非常繁琐. 以下的结果是如此计算的：
...
\end{axis}
\end{tikzpicture}}
        译注：可以参考小谷正雄, 桥本英典（日）所著《特殊函数》, p.36. 书中指出Gegenbauer(盖根鲍尔)多项式是Jacobi多项式的特殊情形, 继续特殊化还可得到Legendre多项式和Tschebyscheff多项式.
        王竹溪《特殊函数》中详细讨论了Jacobi多项式及其退化型, 当然也就包括盖根鲍尔多项式(啊! Hypergeometric Function, My Love!).
        另外一处参考就是两位著名物理学家郝柏林和陈式刚教授的论文：郝柏林,刘德森,陈式刚. 长共轭键链的$\pi$电子能谱. 物理学报, 1961, 17(7): 303.
\end{enumerate}
}
...
\let\thesection\oldthesection
\label{sec6.4}
\autoref{sec6.2}为RS微扰论引入了一种完全普适的图表示.
为了利用它来处理轨道微扰,
我们用向上、下的线来分别表示空穴、粒子的自旋轨道,
用点来对应单粒子微扰$v$.
然后画出和之前同样的一组图, 用指标$a$, $b$, ...来标记空穴, 用$r$, $s$, ...来标记粒子. 所以就有
\begin{center}	
	...
	\path [draw=blue,postaction={on each segment={mid arrow}}]
	(2,-7.) node{\dian}
	arc (-45:45:2.12cm)node[right,midway]{r} node{\dian} arc(135:165:2.12cm)node[left,midway]{a} node{\dian}
        arc(165:195:2.12cm)node[left,midway]{b} node{\dian} arc(195:225:2.12cm)node[left,midway]{c};
	...
	\path [draw=blue,postaction={on each segment={mid arrow}}]
	(6,-7.) node{\dian}
	arc (-45:-15:2.12cm)node[right,midway]{s} node{\dian} arc(-15:45:2.12cm)node[right,midway]{r} node{\dian}
        arc(135:165:2.12cm)node[left,midway]{a} node{\dian} arc(165:225:2.12cm)node[left,midway]{b};
	
	\path [draw=blue,postaction={on each segment={mid arrow}}]
	(8,-7.) node{\dian}
	arc (-45:15:2.12cm)node[right,midway]{r} node{\dian} arc(-15:-45:2.12cm)node[left,midway]{b} node{\dian}
        arc(225:180:2.57cm)node[right,midway]{s} node{\dian} to [bend right] node[left,midway]{s} (8,-7.);
	

	\path [draw=blue,postaction={on each segment={mid reverse arrow}}]
	(10,-7.) node{\dian}
	arc (-45:15:2.12cm)node[right,midway]{r} node{\dian} arc(-15:-45:2.12cm)node[left,midway]{b} node{\dian}
        arc(225:180:2.57cm)node[right,midway]{s} node{\dian} to [bend right] node[left,midway]{s} (10,-7.);
	\draw (12,-5.5)node{\normalsize $n=4$};
	\end{tikzpicture}
\end{center}
...
\begin{enumerate}[a.]
	\item 证明
		...
		\path [draw=blue,postaction={on each segment={mid arrow}}]
		(6,-7.) node{\dian}
		arc (-45:-15:2.12cm)node[right,midway]{s} node{\dian}
                arc(-15:45:2.12cm)node[right,midway]{r} node{\dian}
                arc(135:165:2.12cm)node[left,midway]{a} node{\dian}
                arc(165:225:2.12cm)node[left,midway]{b};
		\end{tikzpicture}
		=		
		\begin{tikzpicture}[baseline={(current bounding box.center)}, postaction={on each segment={mid arrow}},scale=.6]
			\path [draw=blue,postaction={on each segment={mid arrow}}]
			(2,-7.) node{\dian}
			arc (-45:45:2.12cm)node[right,midway]{r} node{\dian}
                        arc(135:165:2.12cm)node[left,midway]{a} node{\dian}
                        arc(165:195:2.12cm)node[left,midway]{b} node{\dian}
                        arc(195:225:2.12cm)node[left,midway]{c};\end{tikzpicture}
		=
		\begin{tikzpicture}[baseline={(current bounding box.center)}, postaction={on each segment={mid arrow}},scale=.6]
			\path [draw=blue,postaction={on each segment={mid arrow}}]
			(0,-7.) node{\dian}
		...
		\begin{tikzpicture}[baseline={(current bounding box.center)}, postaction={on each segment={mid arrow}},scale=.6]
			\path [draw=blue,postaction={on each segment={mid arrow}}]
			(8,-7.) node{\dian}
			arc (-45:15:2.12cm)node[right,midway]{r} node{\dian} arc(-15:-45:2.12cm)node[left,midway]{b} node{\dian} arc(225:180:2.57cm)node[right,midway]{s} node{\dian} to [bend right] node[left,midway]{a} (8,-7.);
		\end{tikzpicture}
		=
		\begin{tikzpicture}[baseline={(current bounding box.center)}, postaction={on each segment={mid arrow}},scale=.6]
		\path [draw=blue,postaction={on each segment={mid reverse arrow}}]
		(10,-7.) node{\dian}
		arc (-45:15:2.12cm)node[right,midway]{b} node{\dian} arc(-15:-45:2.12cm)node[left,midway]{s} node{\dian} arc(225:180:2.57cm)node[right,midway]{a} node{\dian} to [bend right] node[left,midway]{r} (10,-7.);
	...
	\item 对于苯, 证明用图形算出的四阶能量与\autoref{ex:6.6}中独立得出的值完全相同. 
\end{enumerate}
}
...
二阶能量的一般表达式在\autoref{sec6.1}已经推导过, 
为
\begin{align}
...
二阶能量可以写成空间轨道的求和:
\begin{align}
E_0^{(2)} = 2\sum_{abrs}^{N/2}\frac{\braket{ab|rs}\braket{rs|ba}}{\epsilon_a + \epsilon_b -\epsilon_r-\epsilon_s}
    -\sum_{abrs}^{N/2}\frac{\braket{ab|rs}\braket{rs|ba}}{\epsilon_a + \epsilon_b -\epsilon_r-\epsilon_s}
\label{6.74}
\end{align}
\exercise{
...
E_0^{(3)} = & \frac{1}{8} \sum_{abcdrs} \frac{\braket{ab||rs}\braket{cd||ab}\braket{rs||cd}}{(\epsilon_a + \epsilon_b -\epsilon_r-\epsilon_s)(\epsilon_c + \epsilon_d -\epsilon_r-\epsilon_s)} \notag\\
          & + \frac{1}{8} \sum_{abrstu} \frac{\braket{ab||rs}\braket{rs||tu}\braket{tu||ab}}{(\epsilon_a + \epsilon_b -\epsilon_r-\epsilon_s)(\epsilon_a + \epsilon_b -\epsilon_t-\epsilon_u)} \notag\\
...
\end{align*}
若将上面的相关能表达式按双电子积分的Taylor级数\footnote{
译者注: 即将相关能视为以双电子积分为自变量的多元函数,
展开成泰勒级数. 可以适当合并齐次自变量以减少冗余自由度、
少算几个偏导数.
}展开, 
展开到第三阶, 
可以发现
...
该定理用RS微扰论的图表示证明了Brueckner的猜想适用于所有阶的微扰. 
...
这个超分子的\hft 能量就是
\begin{equation}
E_0=\braket{\Psi_0|\hs_0+\vs|\Psi_0} = E_0^{(0)} + E_0^{(1)} = N(2\epsilon_1-J_{11})
\end{equation}
...
式中
\begin{gather}
A_0^{(3)} = \sideset{}{'}\sum_n \sideset{}{'}\sum_m \frac{\braket{0|\vs|n}\braket{n|\vs|m}\braket{m|\vs|0}}{(E_0^{(0)} - E_n^{(0)}) (E_0^{(0)} - E_m^{(0)}) }\\
...
注意到\autoref{6.78}是在基组内按照Taylor级数得到的, 
这两种办法给出的结果相同, 
那么他们的一致性就得到了检验. 
虽然这个例子算不上证明, 
...
\begin{enumerate}[H1.]
	\kaishu
	\item 每个点在分母上贡献一个反对称的矩阵元
            $\langle\allowbreak\text{穿入标签1},\allowbreak\text{穿入标签2}\allowbreak||\allowbreak\text{穿出标签1},\allowbreak\text{穿出标签2}\allowbreak\rangle$.
            % 不用braket是因为中间不能换行
            具体谁是1、谁是2的顺序可任意指定.
	\item
	每一对相邻点在分母上贡献一个因子：
	...
	\sum \epsilon_\text{粒子} 
	\end{align*} 
	在相邻两点间作一条水平虚线, 式中的求和遍及所有与虚线相交的空穴线和粒子线.
	\item 
	表达式的总符号为$(-)^{h+l}$,$h$和$l$分别是空穴线和闭合圈的数目.
        闭合圈数目不能单单直接从图中看出来.后面会讲如何从(H1)指定的各点指标顺序得到这个数目.
	\item 
	将表达式对所有粒子和空穴指标求和.
	\item 将式子乘以权重因子$2^{-k}$, $k$代表图中有$k$对等价线.
            等价线的意思是两条线的出发点、结束点、方向都一样.
\end{enumerate}
可以看见仅有(H5)和之前的规则明显不同. 
具体运用这些规则时, 
唯一比较难办的是确定闭合圈的数目. 
具体的做法请看下例. 
考虑这个二阶图:
...
按如下规则``走过''矩阵元:
\begin{align*}
\braket{i\tikzmark{i}j||k\tikzmark{k}l}\quad \text{和}\quad
...
\end{align*}
需要两个圈$a\to r\to c\to a$和$b\to s\to d\to b$, 
因此$l$为$2$.
...
\intertext{$l=2$, 因为要$a\to r\to t\to a$和$b\to s\to u \to b$两圈. 最后,}
\begin{tikzpicture}[baseline={(current bounding box.east)},scale=.6]
% 下面第3个图的结点
\coordinate (a) at (0,-4);
\coordinate (b) at (0,-2.5);
...
\end{center}
	等于
	\begin{align*}
	-\frac{1}{2}\sum_{abcderst}\frac{\braket{rs||ac}\braket{at||de}\braket{dc||tb}\braket{eb||rs}}{(\epsilon_{a}+\epsilon_{c}-\epsilon_{r}-\epsilon_{s})(\epsilon_{c}+\epsilon_{d}+\epsilon_{e}-\epsilon_{r}-\epsilon_{t}-\epsilon_{s})(\epsilon_{b}+\epsilon_{e}-\epsilon_{r}-\epsilon_{s})}
	\end{align*}
}

\subsection{Goldstone图}\label{sec6.7.2}
本节介绍多体微扰论的另外一种图表示：Goldstone引入的Goldstone图.
...
\autoref{t6.2}中列出了\hft 微扰论中所有的二阶和三阶Golstone图. 计算这些图的规则是:
\begin{enumerate}[G1.]
	\item 每条相互作用线:
	\begin{center}
		\begin{tikzpicture}[baseline={(current bounding box.south)},scale=.6]
...
\begin{tabular}{ccl}
    ...
    \path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
    \end{tikzpicture} 
    & $\displaystyle(-)^{2+2}\left(\frac{1}{2}\right)\sum_{abrsut}
    \frac{\braket{ab|ru}\braket{ru|ts}\braket{ts|ab}}
    {(\epsilon_{a}+\epsilon_{b}-\epsilon_{r}-\epsilon_{u})
    (\epsilon_{a}+\epsilon_{b}-\epsilon_{t}-\epsilon_{s})}$\\\hline
    2 & \begin{tikzpicture}[baseline={(current bounding box.center)},scale=.75]
    \coordinate (a) at (4,-2.5+2);
    \coordinate (b) at ($(a)+(2,0)$);
    \coordinate (c) at ($(a)+(0,-2)$);
    ...
    \path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
    \end{tikzpicture} 
    & $\displaystyle(-)^{4+2}\left(\frac{1}{2}\right)\sum_{abcdrs}
    \frac{\braket{ad|rs}\braket{cb|ad}\braket{rs|cb}}
    {(\epsilon_{a}+\epsilon_{d}-\epsilon_{r}-\epsilon_{s})(\epsilon_{c}+\epsilon_{b}-\epsilon_{r}-\epsilon_{s})}$\\\hline
    ...
    \path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
    \end{tikzpicture} 
    & $\displaystyle(-)^{3+3}\sum_{abcrst}
    \frac{\braket{ac|rt}\braket{bt|sc}\braket{rs|ab}}
    {(\epsilon_{a}+\epsilon_{c}-\epsilon_{r}-\epsilon_{t})(\epsilon_{a}+\epsilon_{b}-\epsilon_{r}-\epsilon_{s})}$\\\hline
\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{ccl}\hline
		...
		\path[mid arrow seg,draw=blue]
		(a) to node[midway,right]{$a$} (m) 
		(m) to node[midway,right]{$s$} (b)
		(b) arc(45:-45:1.414) node[midway,right]{$b$};
		\path 
		(d) arc(225:180:1.414) coordinate (n);
		\path[mid arrow seg,draw=blue]
		(d) to node[midway,left]{$t$} (n)
		(n) to node[midway,right]{$c$} (c);
		\draw[draw=blue,densely dotted]
		(a)--(b)
		(c)--(d)
		(m)--(n);
		...
		;
		\path[mid arrow seg=.75,draw=blue]
		(c) to node[pos=.75,right]{$r$} (b) 
		(d) to node[pos=.75,left]{$s$}  (a) 
		;
		\draw[draw=blue,densely dotted]
		(a)--(b)
		(c)--(d)
		...
		\path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
		\end{tikzpicture}
		& $\displaystyle(-)^{2+1}\left(\frac{1}{2}\right)\sum_{abrsut}
                \frac{\braket{ab|rt}\braket{tr|us}\braket{us|ab}}
                {(\epsilon_{a}+\epsilon_{b}-\epsilon_{t}-\epsilon_{r})
                (\epsilon_{a}+\epsilon_{b}-\epsilon_{u}-\epsilon_{s})}$\\\hline
		...
		;
		\path[mid arrow seg=.75,draw=blue]
		(c) to node[midway,right]{$r$} (-1.36597012977+2,-1.36597012977) coordinate (n) % -1.36597012977是点(m)的纵座标
		...
		\path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
		\end{tikzpicture}
		& $\displaystyle(-)^{3+1}\left(\frac{1}{2}\right)\sum_{abcrst}
                \frac{\braket{cb|rt}\braket{ra|sb}\braket{st|ac}}
                {(\epsilon_{c}+\epsilon_{b}-\epsilon_{r}-\epsilon_{t})(\epsilon_{a}+\epsilon_{c}-\epsilon_{s}-\epsilon_{t})}$\\\hline
		...
		\path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
		\end{tikzpicture} 
		& $\displaystyle(-)^{3+2}\sum_{abcrst}
                \frac{\braket{ab|rs}\braket{sc|at}\braket{rt|bc}}
                {(\epsilon_{a}+\epsilon_{b}-\epsilon_{r}-\epsilon_{s})
                (\epsilon_{c}+\epsilon_{b}-\epsilon_{r}-\epsilon_{t})}$\\\hline
...
\exercise{把二阶/三阶Hugenholtz图拉开, 就能得到\autoref{t6.2}中所列的这些Goldstone图. 这个过程可能不那么容易看出来, 但是逆过程很容易理解. 比如, 把如下的图挤压成一个图:
	\begin{align*}
	\tikz[baseline={(current bounding box.center)},scale=.6]{
		\coordinate (a) at (4,-2.5+2);
		\coordinate (b) at ($(a)+(2,0)$);
...
很容易看出结果是下图:
	...
	\end{align*}
同样地, 
把\autoref{t6.2}中的三阶Goldstone图压在一起, 
就能得到对应的Hugenholtz图. 
请你验证, 
...
下面我们举个例子, 
在极小基$\hd$ 模型下计算\autoref{t6.2}中二阶和三阶图的值, 
我们会看见所得值与之前的结果完全一样(\autoref{6.77} \eqref{6.78}). 
我们首先转换到对空间轨道的求和, 
这一步要乘以$2^l$, 
$l$是闭合圈的数目. 
然后把所有空穴指标都标为$1$, 
...
%	(m) arc(0:45:1.414)node[midway,right]{$t$}
	(d) arc(225:135:1.414) coordinate(n) node[midway,left]{$s$}
%	(n) arc(180:135:1.414) node[midway,left]{$s$}
	(b) arc(45:-45:1.414) node[midway,right]{$b$}
	; 
	\draw[draw=blue,densely dotted]
...
\tikz[baseline={(current bounding box.center)},scale=.6]{
	...
	(a) arc(135:225:1.414) node[midway,left]{$a$}  
	(c) arc(-45:0:1.414) coordinate(m) node[midway,right]{$r$} 
	(m) arc(0:45:1.414)node[midway,right]{$r$}
	(d) arc(225:180:1.414) coordinate(n) node[midway,left]{$s$}
	(n) arc(180:135:1.414) node[midway,left]{$s$}
	(b) arc(45:-45:1.414) node[midway,right]{$b$}
	; 
	\draw[draw=blue,densely dotted]
...
最后我们再提一个例子, 
如果把所有\emph{双激发}图(所有水平虚线仅穿过两条空穴线、两个粒子线的图)加和起来, 
...
\subsection{什么是连通簇（Linked-Cluster）定理?}
Goldstone在他的\mci{连通簇}(linked-cluster)定理中用图形技术证明,
在更一般的情况下, Brueckner猜想依然成立,
也即, RS微扰论的每一阶都尺寸一致(size-consistent).
正比于$N^2$的项所对应的图由分离的几部分构成（即一个\emph{非连通}图）,  
Goldstone用含时技巧证明了这类图不会出现在$n$阶能量的最终表达式中. 
因此他把$n$阶能量纯粹用\phrase{连通}图表示了出来. 本章所用的图全部是\phrase{连通图}. 
之前我们已经多次对比了图方法和代数方法所得的两种结果, 它们完全一致, 
读者可以相信RS微扰论的所有形式(即$N$-态微扰论、轨道微扰论等等形式)都能单纯用\phrase{连通图}表示出来（或者至少相信这个说法的合理性）. 
这里要说明一下``尺寸一致''这个词,
本书中说某个理论尺寸一致时, 意思是,
该理论给出的多体系统能量随系统大小线性增长.
仅当系统是$N$个独立闭壳层单体组成的超分子 且 所用的方法在简并轨道的酉变换下不变时,
尺寸一致这个词才意味着系统的能量是$N$倍的单体能量.
\exercise{
某个超分子由$N$个无相互作用的极小基$\hd$分子组成, 
请通过计算\autoref{t6.2}中的Goldstone图, 
得到该系统的$E_0^{(3)}$. 
将你的结果与\autoref{eq:6.92}对比,
...
\begin{table}
	...
	\begin{tabular}{lccccc}
		...
		\hline
		$\ch{CH4}$ & & & & & \\
		\multicolumn{1}{c}{\quad SCF} & $2.047$ & $2.043$ & $2.048$ & $2.048$ & \multirow{2}{*}{$2.050$}\\
                \multicolumn{1}{c}{\quad $E_0^{(2)}$} & $2.077$ & $2.065$ & $2.060$ & $2.048$ & \\
		$\ch{NH3}$ & & & & & \\
                \multicolumn{1}{c}{\quad SCF} & $1.952$ & $1.873$ & $1.897$ & $1.897$ & \multirow{2}{*}{$1.913$}\\
                \multicolumn{1}{c}{\quad $E_0^{(2)}$} & $1.997$ & $1.907$ & $1.922$ & $1.912$ &  \\
		$\ch{H2O}$ & & & & & \\
                \multicolumn{1}{c}{\quad SCF} & $1.871$ & $1.797$ & $1.791$ & $1.782$ & \multirow{2}{*}{$1.809$}\\
                \multicolumn{1}{c}{\quad $E_0^{(2)}$} & $1.916$ & $1.842$ &  $1.831$ & $1.816$ & \\
		$\ch{FH}$ & & & & & \\ 
                \multicolumn{1}{c}{\quad SCF} & $1.812$ & $1.742$ & $1.722$ & $1.703$ & \multirow{2}{*}{$1.733$}\\
                \multicolumn{1}{c}{\quad $E_0^{(2)}$} & $1.842$ & $1.790$ & $1.765$ & $1.740$ & \\ \hline
	\end{tabular}
	\label{tab:6.5}
\end{table}
...
\begin{table}
	...
	\begin{tabular}{cccccc}
		...
		\hline $\ch{NH3}$ & & & & & \\
                \multicolumn{1}{c}{\quad SCF} & $104.2$ & $115.8$ & $107.5$ & $107.6$ & \multirow{2}{*}{106.7} \\
                \multicolumn{1}{c}{\quad $E_0^{(2)}$} & $100.9$ & $113.9$ & $106.3$ & $106.1$ & \\
		$\ch{H_2O}$ & & & & & \\
                \multicolumn{1}{c}{\quad SCF} & $100.0$ & $111.2$ & $105.5$ & $106.0$ & \multirow{2}{*}{$104.5$} \\
                \multicolumn{1}{c}{\quad $E_0^{(2)}$} & $97.2$ & $108.8$ & $104.0$ & $103.9$ & \\
		\hline
	\end{tabular}
	\end{threeparttable}
	\label{tab:6.6}
...
\begin{table}
	\centering
        \caption{$\ch{H_2O}$在实验测量的几何形状下的关联能；基组为第四章中介绍的39-STO$^\text{a}$\tnote{a}}
	\begin{threeparttable}
...
\begin{table}
	...
	\begin{threeparttable}
		\begin{tabular}{lcccc}
                    \hline & $R_e$ (a.u.) & $\theta_e$ & $f_{R R}$\tnote{b} & $f_{\theta \theta}\tnote{b}$ \\
...
用标准基组的二阶能量计算得到的偶极矩如\autoref{tab:6.10}所示.  
...
\begin{table}
	...
	\begin{tabular}{lcccccccc}
		...
		\cline { 2 - 3 } \cline { 5 - 6 } \cline { 8 - 9 } 基组 & $\mathrm{SCF}$ & $E_0^{(2)}$ & & $\mathrm{SCF}$ & $E_0^{(2)}$ & & $\mathrm{SCF}$ & $E_0^{(2)}$ \\
...
二阶能量水平上的微扰论能克服这一点吗？\autoref{tab:6.11} 表明, 
较大的基组定性地经受住了考验（即, 该基组下, SCF偶极矩和\phrase{近HF极限}一致给出错误的符号）. 
...
虽然这绝不是显而易见的, 但答案是, 虽然二阶能量确实只包含双激发, 但用于计算 $E_2^{(0)}$ 的 HF 轨道依赖于场；
